...
impl std::fmt::Display for BString {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    unsafe {
      write!(f,
             "{}",
             to_os_string(self.0).into_string()
    ...
    ($name:ident, $type:ty, $release_func:ident) => {
        ...
        impl $name {
            pub(crate) unsafe fn new(
                type_info: std::ptr::NonNull<winapi::um::oaidl::ITypeInfo>,
                ptr: std::ptr::NonNull<$type>,
            ) -> Self {
                $name {
                    type_info: ComRc::new(type_info),
                    ptr,
                }
            }
        }
...
type_info_associated_rc!(TypeAttributesRc,
                         winapi::um::oaidl::TYPEATTR,
                         ReleaseTypeAttr);
type_info_associated_rc!(VarDescRc,
                         winapi::um::oaidl::VARDESC,
                         ReleaseVarDesc);
type_info_associated_rc!(FuncDescRc,
                         winapi::um::oaidl::FUNCDESC,
                         ReleaseFuncDesc);

unsafe fn to_os_string(bstr: winapi::shared::wtypes::BSTR)
                       -> std::ffi::OsString {
