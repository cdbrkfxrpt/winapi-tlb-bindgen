...
pub struct BuildResult {
  /// The number of referenced types that could not be found and were replaced
  /// with `__missing_type__`
  pub num_missing_types: usize,

  /// The number of types that could not be found
  pub num_types_not_found: usize,

  /// The number of dispinterfaces that were skipped because the
  /// `emit_dispinterfaces` parameter of [`build`] was false
...
/// Parses the typelib (or DLL with embedded typelib resource) at the given
/// path and emits bindings to the given writer.
pub fn build<W>(filename: &std::path::Path,
  ...
  where W: std::io::Write
{
  let mut build_result =
    BuildResult { num_missing_types: 0,
  ...
  unsafe {
    ...
    let type_lib = {
      ...
      error::to_result(winapi::um::oleauto::LoadTypeLibEx(
                filename.as_ptr(),
                winapi::um::oleauto::REGKIND_NONE,
                &mut type_lib_ptr,
            ))?;
      let type_lib =
        types::TypeLib::new(std::ptr::NonNull::new(type_lib_ptr).unwrap());
    ...
    for type_info in type_lib.get_type_infos() {
      ...
      let type_info = if type_info.attributes().typekind
        ...
        match type_info.get_interface_of_dispinterface() {
                    Ok(disp_type_info) => {
                        build_result
                            ...
                            .push(format!("{}", type_info.name()));
                        disp_type_info
                    }

                    Err(Error::HResult(winapi::shared::winerror::TYPE_E_ELEMENTNOTFOUND)) => {
                        type_info
      ...
      match attributes.typekind {
          ...
          for field in type_info.get_fields() {
            ...
            writeln!(out,
                     ...
                     type_to_string(field.type_(),
                                    winapi::um::oaidl::PARAMFLAG_FOUT,
                                    &type_info,
                                    &mut build_result)?)?;
          }
          ...
          for function in type_info.get_functions() {
            ...
            for param in function.params() {
              ...
              writeln!(
                                ...
                                type_to_string(
                                    &param_desc.tdesc,
                                    winapi::shared::minwindef::DWORD::from(
                                        param_desc.u.paramdesc().wParamFlags
                                    ),
                                    &type_info,
                                    &mut build_result
                                )?
            ...
            writeln!(out,
                     ") -> {},",
                     type_to_string(&function_desc.elemdescFunc.tdesc,
                                    winapi::um::oaidl::PARAMFLAG_FOUT,
                                    &type_info,
                                    &mut build_result)?)?;
            writeln!(out)?;
        ...
        winapi::um::oaidl::TKIND_INTERFACE => {
          writeln!(out,
                   "RIDL!{{#[uuid(0x{:08x}, 0x{:04x}, 0x{:04x}, 0x{:02x}, \
                    0x{:02x}, 0x{:02x}, 0x{:02x}, 0x{:02x}, 0x{:02x}, \
                    0x{:02x}, 0x{:02x})]",
                   attributes.guid.Data1,
                   attributes.guid.Data2,
                   attributes.guid.Data3,
                   attributes.guid.Data4[0],
                   attributes.guid.Data4[1],
                   attributes.guid.Data4[2],
                   attributes.guid.Data4[3],
                   attributes.guid.Data4[4],
                   attributes.guid.Data4[5],
                   attributes.guid.Data4[6],
                   attributes.guid.Data4[7])?;
          ...
          for function in type_info.get_functions() {
            ...
            assert_ne!(function_desc.funckind,
                       winapi::um::oaidl::FUNC_DISPATCH);
            ...
            match function_desc.invkind {
              ...
              winapi::um::oaidl::INVOKE_PROPERTYGET => {
                ...
                for param in function.params() {
                  ...
                  if (winapi::shared::minwindef::DWORD::from(
                                        param_desc.u.paramdesc().wParamFlags,
                                    ) & winapi::um::oaidl::PARAMFLAG_FRETVAL)
                                        == winapi::um::oaidl::PARAMFLAG_FRETVAL
                                    {
                                        assert_eq!(
                                            function_desc.elemdescFunc.tdesc.vt,
                                            winapi::shared::wtypes::VT_HRESULT
                                                as winapi::shared::wtypes::VARTYPE
                                        );
                ...
                if explicit_ret_val {
                  assert_eq!(function_desc.elemdescFunc.tdesc.vt,
                             winapi::shared::wtypes::VT_HRESULT
                             as winapi::shared::wtypes::VARTYPE);
                  writeln!(
                                        out,
        ...
        winapi::um::oaidl::TKIND_DISPATCH => {
          if !emit_dispinterfaces {
            build_result.skipped_dispinterfaces
                        .push(format!("{}", type_info.name()));
            continue;
          }
...

          {
            ...
            if let Some(Ok(parent)) = parents.next() {
              ...
              assert_eq!(parent.attributes().cbSizeVft as usize,
                         7 * std::mem::size_of::<usize>()); // 3 from IUnknown + 4 from IDispatch
            } else {
              unreachable!();
            }
          ...
          let has_inherited_functions =
            type_info.get_functions()
                     .any(|function| function.unwrap().desc().oVft > 0);
          ...
          for function in type_info.get_functions() {
            ...
            assert_eq!(function_desc.funckind,
                       winapi::um::oaidl::FUNC_DISPATCH);

            if has_inherited_functions
               && (function_desc.oVft as usize)
            ...
            let params: Vec<_> =
              function.params()
                      ...
                      .filter(|param| {
                        (winapi::shared::minwindef::DWORD::from(
                                    param.desc().u.paramdesc().wParamFlags,
                                ) & winapi::um::oaidl::PARAMFLAG_FRETVAL)
                                    == 0
                      })
                      .collect();
            ...
            if !params.is_empty() {
              ...
              for param in params.into_iter().rev() {
                let param_desc = param.desc();
                if (winapi::shared::minwindef::DWORD::from(param.desc()
                                                                .u
                                                                .paramdesc()
                                                                .wParamFlags)
                    & winapi::um::oaidl::PARAMFLAG_FRETVAL)
                ...
                {
                  let (vt, mutator) =
                    vartype_mutator(&param_desc.tdesc,
                                    &sanitize_reserved(param.name()),
                                    &type_info);
                  writeln!(out,
                           "            {{ let mut v: VARIANT = \
                            ::core::mem::uninitialized(); VariantInit(&mut \
                            v); *v.vt_mut() = {}; *v{}; v }},",
                           vt, mutator)?;
                }
              }
            ...
            if function_desc.invkind == winapi::um::oaidl::INVOKE_PROPERTYPUT
               || function_desc.invkind
                  == winapi::um::oaidl::INVOKE_PROPERTYPUTREF
            {
            ...
            writeln!(out)?;
            writeln!(out,
                     "        let mut exception_info: EXCEPINFO = \
                      ::core::mem::zeroed();")?;
            writeln!(out)?;
          ...
          for property in type_info.get_fields() {
            ...
            writeln!(out,
                     "            {{ let mut v: VARIANT = \
                      ::core::mem::uninitialized(); VariantInit(&mut v); \
                      *v.vt_mut() = {}; *v{}; v }},",
                     vt, mutator)?;
            writeln!(out, "        ];")?;
            writeln!(out)?;
            ...
            writeln!(out,
                     "            rgdispidNamedArgs: \
                      ::core::ptr::null_mut(),")?; // TODO: PROPERTYPUT needs named args?
            writeln!(out, "            cArgs: 1,")?;
            writeln!(out, "            cNamedArgs: 0,")?;
            writeln!(out, "        }};")?;
            writeln!(out)?;
        ...
        winapi::um::oaidl::TKIND_ALIAS => {
          writeln!(out,
                   "pub type {} = {};",
                   type_name,
                   type_to_string(&attributes.tdescAlias,
                                  winapi::um::oaidl::PARAMFLAG_FOUT,
                                  &type_info,
                                  &mut build_result)?)?;
          writeln!(out)?;
...
unsafe fn type_to_string(type_: &winapi::um::oaidl::TYPEDESC,
                         param_flags: u32,
  ...
  match winapi::shared::wtypes::VARENUM::from(type_.vt) {
        winapi::shared::wtypes::VT_PTR => {
            if (param_flags & winapi::um::oaidl::PARAMFLAG_FIN) == winapi::um::oaidl::PARAMFLAG_FIN
                && (param_flags & winapi::um::oaidl::PARAMFLAG_FOUT) == 0
            {
                // [in] => *const
                type_to_string(&**type_.u.lptdesc(), param_flags, type_info, build_result)
        ...
        winapi::shared::wtypes::VT_CARRAY => {
            ...
            let dimensions: &[winapi::um::oaidl::SAFEARRAYBOUND] =
                std::slice::from_raw_parts((**type_.u.lpadesc()).rgbounds.as_ptr(), num_dimensions);

            let mut type_name = type_to_string(
                &(**type_.u.lpadesc()).tdescElem,
                param_flags,
                type_info,
                build_result,
            )?;
        ...
        winapi::shared::wtypes::VT_USERDEFINED => match type_info
            .get_ref_type_info(*type_.u.hreftype())
...
fn well_known_type_to_string(vt: winapi::shared::wtypes::VARTYPE)
                             -> &'static str {
...
unsafe fn vartype_mutator(type_: &winapi::um::oaidl::TYPEDESC,
                          param_name: &str,
                          type_info: &types::TypeInfo)
                          -> (winapi::shared::wtypes::VARENUM, String) {
  match winapi::shared::wtypes::VARENUM::from(type_.vt) {
    vt @ winapi::shared::wtypes::VT_I2 => {
      (vt, format!(".iVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_I4 => {
      (vt, format!(".lVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_CY => {
      (vt, format!(".cyVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_BSTR => {
      (vt, format!(".bstrVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_DISPATCH => {
      (vt, format!(".pdispVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_ERROR => {
      (vt, format!(".scode_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_BOOL => {
      (vt, format!(".boolVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_VARIANT => {
      (vt, format!(" = *(&{} as *const _ as *mut _)", param_name))
    }
    vt @ winapi::shared::wtypes::VT_UNKNOWN => {
      (vt, format!(".punkVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_UI2 => {
      (vt, format!(".uiVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_UI4 => {
      (vt, format!(".ulVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_INT => {
      (vt, format!(".intVal_mut() = {}", param_name))
    }
    vt @ winapi::shared::wtypes::VT_UINT => {
      (vt, format!(".uintVal_mut() = {}", param_name))
    }
    winapi::shared::wtypes::VT_PTR => {
      let pointee_vt =
        winapi::shared::wtypes::VARENUM::from((**type_.u.lptdesc()).vt);
      match pointee_vt {
        winapi::shared::wtypes::VT_I4 => {
          (pointee_vt | winapi::shared::wtypes::VT_BYREF,
           format!(".plVal_mut() = {}", param_name))
        }
        winapi::shared::wtypes::VT_BSTR => {
          (pointee_vt | winapi::shared::wtypes::VT_BYREF,
           format!(".pbstrVal_mut() = {}", param_name))
        }
        winapi::shared::wtypes::VT_DISPATCH => {
          (pointee_vt | winapi::shared::wtypes::VT_BYREF,
           format!(".ppdispVal_mut() = {}", param_name))
        }
        winapi::shared::wtypes::VT_BOOL => {
          (pointee_vt | winapi::shared::wtypes::VT_BYREF,
           format!(".pboolVal_mut() = {}", param_name))
        }
        winapi::shared::wtypes::VT_VARIANT => {
          (pointee_vt | winapi::shared::wtypes::VT_BYREF,
           format!(".pvarval_mut() = {}", param_name))
        }
        winapi::shared::wtypes::VT_USERDEFINED => {
          (winapi::shared::wtypes::VT_DISPATCH,
           format!(".pdispVal_mut() = {}", param_name))
        }
        _ => unreachable!(),
      }
    }
    winapi::shared::wtypes::VT_USERDEFINED => {
      ...
      match size {
        4 => (winapi::shared::wtypes::VT_I4,
              format!(".lVal_mut() = {}", param_name)), // enum
        _ => unreachable!(),
      }
    }
    _ => unreachable!(),
