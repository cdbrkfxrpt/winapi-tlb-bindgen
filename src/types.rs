...
impl TypeLib {
  pub(crate) unsafe fn new(ptr: std::ptr::NonNull<winapi::um::oaidl::ITypeLib>)
                           -> Self {
...
impl<'a> Iterator for TypeInfos<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    unsafe {
      ...
      let result =
                crate::error::to_result(self.type_lib.GetTypeInfo(self.index, &mut type_info))
                    .and_then(|_| {
...
impl TypeInfo {
  pub(crate) unsafe fn new(ptr: std::ptr::NonNull<winapi::um::oaidl::ITypeInfo>)
    ...
    Ok(TypeInfo {
            ptr: crate::rc::ComRc::new(ptr),
            name,
            type_attr: crate::rc::TypeAttributesRc::new(
                ptr,
                std::ptr::NonNull::new(type_attr).unwrap(),
            ),
        })
  }
  ...
  pub(crate) unsafe fn get_ref_type_info(&self,
                                         ref_type: winapi::um::oaidl::HREFTYPE)
                                         -> Result<Self, crate::Error> {
    let mut ref_type_info = std::ptr::null_mut();
    crate::error::to_result(self.ptr.GetRefTypeInfo(ref_type,
                                                    &mut ref_type_info))?;
  ...
  pub(crate) unsafe fn get_interface_of_dispinterface(
    &self)
    -> Result<Self, crate::Error> {
    let mut ref_type = 0;
    crate::error::to_result(
            self.ptr
                .GetRefTypeOfImplType(-1_i32 as winapi::shared::minwindef::UINT, &mut ref_type),
        )?;

    let mut type_info = std::ptr::null_mut();
    crate::error::to_result(self.ptr
                                .GetRefTypeInfo(ref_type, &mut type_info))?;
...
impl<'a> Vars<'a> {
  pub(crate) unsafe fn new(type_info: &'a winapi::um::oaidl::ITypeInfo,
                           attributes: &winapi::um::oaidl::TYPEATTR)
                           -> Self {
    Vars { type_info,
...
impl<'a> Iterator for Vars<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    unsafe {
      let result = Var::new(self.type_info.into(),
                            winapi::shared::minwindef::UINT::from(self.index));
      self.index += 1;
      Some(result)
    }
  }
...
impl Var {
  pub(crate) unsafe fn new(type_info: std::ptr::NonNull<winapi::um::oaidl::ITypeInfo>,
                           index: winapi::shared::minwindef::UINT)
                           -> Result<Self, crate::Error> {
    let mut desc = std::ptr::null_mut();
    crate::error::to_result((*type_info.as_ptr()).GetVarDesc(index,
                                                             &mut desc))?;
    let desc =
      crate::rc::VarDescRc::new(type_info,
                                std::ptr::NonNull::new(desc).unwrap());

    let name = {
      let mut num_names_received = 0;
      let mut name = std::ptr::null_mut();
      crate::error::to_result((*type_info.as_ptr()).GetNames(
                desc.memid,
                &mut name,
                1,
                &mut num_names_received,
            ))?;
...
impl<'a> Iterator for Functions<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    unsafe {
      let result =
        Function::new(self.type_info.into(),
                      winapi::shared::minwindef::UINT::from(self.index));
      self.index += 1;
      Some(result)
    }
...
impl Function {
  pub(crate) unsafe fn new(type_info: std::ptr::NonNull<winapi::um::oaidl::ITypeInfo>,
                           ...
                           -> Result<Self, crate::Error> {
    let mut desc = std::ptr::null_mut();
    crate::error::to_result((*type_info.as_ptr()).GetFuncDesc(index,
                                                              &mut desc))?;
    let desc =
      crate::rc::FuncDescRc::new(type_info,
                                 std::ptr::NonNull::new(desc).unwrap());
    ...
    crate::error::to_result((*type_info.as_ptr()).GetNames(
            desc.memid,
            names.as_mut_ptr(),
            names.len() as winapi::shared::minwindef::UINT,
            &mut num_names_received,
        ))?;
    ...
    match desc.invkind {
      ...
      | winapi::um::oaidl::INVOKE_PROPERTYGET => {
        assert_eq!(num_names_received,
                   1 + desc.cParams as winapi::shared::minwindef::UINT)
      }

      winapi::um::oaidl::INVOKE_PROPERTYPUT
      | winapi::um::oaidl::INVOKE_PROPERTYPUTREF => {
        if num_names_received
           == desc.cParams as winapi::shared::minwindef::UINT
        {
    ...
    let params =
      names.into_iter()
           .enumerate()
           .map(|(index, name)| Param { name: crate::rc::BString::attach(name),
                                        desc: param_descs.add(index), })
           .collect();
...
impl<'a> Iterator for Parents<'a> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    unsafe {
      ...
      let result = crate::error::to_result(self.type_info.GetRefTypeOfImplType(
                winapi::shared::minwindef::UINT::from(self.index),
                &mut parent_ref_type,
            ))
            .and_then(|_| {
                crate::error::to_result(
                    self.type_info
                        .GetRefTypeInfo(parent_ref_type, &mut parent_type_info),
                )
            })
            .and_then(|_| {
